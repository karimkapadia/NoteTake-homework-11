...
async function loadAndDisplayNotes()
{
  ...
  if( notes.length===0 ){
    ...
    notes.forEach((item) => {
    el_noteList.innerHTML += `
      <li onClick="handleShowNote(event)" id='${item.id}' data-title="${item.title}" data-text="${item.text}" class='list-group-item'><span>${item.title}</span>
      <small onClick="handleNoteDelete(event,'${item.id}')" class='badge bg-secondary float-right'><i class='fas fa-trash-alt icon-resize-small'></i></small>
    `
  });
};
